...
/* Scale the logical game uniformly, keeping its original aspect ratio. */
static void lm_get_transform(int aw,int ah,double *scale,double *offset_x,double *offset_y){double sx,sy;if(lm_logical_width<=0||lm_logical_height<=0||aw<=0||ah<=0){*scale=1.0;*offset_x=*offset_y=0.0;return;}sx=(double)aw/lm_logical_width;sy=(double)ah/lm_logical_height;*scale=sx<sy?sx:sy;*offset_x=(aw-lm_logical_width*(*scale))/2.0;*offset_y=(ah-lm_logical_height*(*scale))/2.0;}
...
void lm_graphics_clear(cairo_t *cr,unsigned int pixel,int width,int height){GtkAllocation a;double scale,ox,oy;lm_logical_width=width;lm_logical_height=height;if(lm_drawing_area&&GTK_IS_WIDGET(lm_drawing_area)){gtk_widget_get_allocation(lm_drawing_area,&a);lm_get_transform(a.width,a.height,&scale,&ox,&oy);}else scale=1.0,ox=oy=0.0;lm_set_source(cr,pixel);cairo_paint(cr);cairo_translate(cr,ox,oy);cairo_scale(cr,scale,scale);}
