...
static void draw_digit(int x, int y, int digit)
{
    static const unsigned char glyph[10][5] = {
        ...
        {0x19,0x15,0x15,0x15,0x13}, {0x11,0x15,0x15,0x15,0x1f},
        {0x07,0x04,0x04,0x04,0x1f}, {0x17,0x15,0x15,0x15,0x1d},
        {0x1f,0x15,0x15,0x15,0x1d}, {0x01,0x01,0x19,0x05,0x03},
...
static void draw(void)
{
    ...
    lm_graphics_fill_rect((LMRect){0, 0, WIDTH, TOP}, 0xffc0c0c0u);
    ...
    for (y = 0; y < ROWS; ++y) {
        for (x = 0; x < COLS; ++x) {
            ...
            if (!c->open) {
                ...
                if (c->flag)
                    lm_graphics_fill_rect((LMRect){px + 8, py + 6, 8, 12}, 0xffff0000u);
            } else if (c->mine) {
...
int main(void)
{
    ...
    if (!lm_graphics_init(WIDTH, HEIGHT, "LinMine"))
        return 1;
