...
static gboolean on_draw(GtkWidget*w,cairo_t*cr,gpointer data)
{
    ...
    face=game_over?(won?BUTTON_SUNGLASSES:BUTTON_DEAD):(face_pressed?(pressed_cell_x>=0?BUTTON_PRESSED:BUTTON_HAPPY_CLICKED):BUTTON_NORMAL);
...
static gboolean on_button_press(GtkWidget*w,GdkEventButton*e,gpointer d)
{
    ...
    if(e->button==GDK_BUTTON_PRIMARY){
        ...
        if(x>=fx&&x<fx+BUTTON_W&&y>=TOP_LED_Y&&y<TOP_LED_Y+BUTTON_H){
            ...
            face_pressed=1;pressed_cell_x=pressed_cell_y=-1;queue_draw();return TRUE;
        }
    }
...
static gboolean on_button_release(GtkWidget*w,GdkEventButton*e,gpointer d)
{
    ...
    if(e->button==GDK_BUTTON_PRIMARY&&face_pressed){
        if(x>=fx&&x<fx+BUTTON_W&&y>=TOP_LED_Y&&y<TOP_LED_Y+BUTTON_H&&pressed_cell_x<0){
        ...
        face_pressed=0;pressed_cell_x=pressed_cell_y=-1;release_mouse_grab(w);queue_draw();return TRUE;
    }
    release_mouse_grab(w);
    return FALSE;
}
